...
rule parse_gerp_beds:
    ...
    output:
      bed = "../results/gerp/{gerp_type}/msacf_a1_{mscaf_nr}_gerp_{gerp_type}.bed.gz"
    conda: "popgen_basics"
    params:
...
rule merge_all_gerp_beds:
    input:
      beds = expand( "../results/gerp/{{gerp_type}}/{mscaf}-gerp-{{gerp_type}}.collapsed.bed.gz", mscaf = SCFS )
    output:
